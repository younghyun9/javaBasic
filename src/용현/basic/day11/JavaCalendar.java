...
public class JavaCalendar {
    public static void main(String[] args) {
        // 자바로 시간,날짜 다루기
        // JDK1에서 제공한 date클래스를 이용해서 날짜,시간에 대한 처리를 할 수 있지만,
        // JDK1.1 에 나온 Calendar 클래스를 이용해서 날짜,시간에 대한 처리를 수행하는 것을 권장함

        // 하지만, 날짜 처리시 윤초를 고려하지 않거나 (정확도 낮음)
        // 월을 표시할 때 1부터 표시하지 않고 0부터 표시하는 문제 존재
        // 따라서 더나은 성능의 Joda-Time 이라는 라이브러리가 JDK8에서 추가됨
        ...
        System.out.printf(fmt, c.get(Calendar.YEAR), c.get(Calendar.MONTH) +1 , c.get(Calendar. DAY_OF_MONTH), c.get(Calendar.HOUR_OF_DAY), c.get(Calendar.MINUTE) , c.get(Calendar.SECOND));
