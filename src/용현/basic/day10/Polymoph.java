...
public class Polymoph {
    public static void main(String[] args) {
        // 클래스 or 인터페이스 간의 형(type) 변환
        // 캐스팅이란 형변환을 의미함
        // 기본 자료형에서는 데이터의 크기에 따라 변환가능
        // 정수와 정수 연산 => 정수
        // 정수와 실수 연산 => 실수

        // 상속관계에 있는 부모/자식 클래스/ 인터페이스 간에
        // 서로 형변환이 가능
        // 이러한 형변환을 업캐스팅/다운캐스팅이라 함
        // 자식클래스의 타입이 부모클래스의 타입으로 바꾸는 것을
        // '업 캐스팅'이라하고, 부모클래스의 타입이
        // 자식 클래스의 타입으로 바뀌는것을 '다운캐스팅' 이라함
        ...
        // 여전히, draw메서드 호출시 동일한 클래스 타입을 사용해야함
        ...
        for(int i=0; i<shapes.length; ++i)
            shapes[i].draw();               // 위의 코드를 for 반복문을 사용해서 단순한게 바꿈

        // binding
        // 프로그램에 사용된 구성요소의 실제 값 또는 속성을 결정하는 행위
        // 즉, 변수에 값이 저장될때나, 함수 호출시 실제함수가 위치한 메모리상의 주소와 연결하는 것을 의미
